<?php
    ...
    if($conn->connect_error){
		...
		if($rsoId_Joined == null){
			$query = $conn->prepare("SELECT RSO_ID, RSO_Name FROM RSO WHERE Uni_Name = ?");
            ...
            $query = $conn->prepare("SELECT RSO_ID, RSO_Name FROM RSO WHERE (RSO_ID NOT IN ($rsoId_Joined)) AND Uni_Name =?");
            $query->bind_param("s", $uniName);
			$query->execute();
